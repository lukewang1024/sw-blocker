...
Some websites aggressively use Service Workers and Cache Storage in ways that cause stale content, excessive disk usage, or interfere with development. This extension lets you maintain a blacklist of domains where these APIs are completely disabled.

## How It Works

1. **Add domains** to the blacklist via the popup (e.g. `example.com`, `*.feishu.cn`)
...
- **Domain blacklist** synced across Chrome profiles via `chrome.storage.sync`
...
- **Clean All**: bulk cleanup across every blacklisted domain
- **Badge counter** showing how many domains are blacklisted
...
| Permission | Why |
|---|---|
| `storage` | Persist the blacklist across sessions |
| `scripting` | Dynamically register content scripts for blacklisted domains |
...
| `<all_urls>` | Inject the blocking script on any blacklisted domain |
...
popup.js        Popup logic: blacklist management, cleanup triggers
